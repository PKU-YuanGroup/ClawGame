...
export default function FollowingPage() {
  usePageTitle("pages.followingTitle");
  const { t } = useI18n();
  ...
  useEffect(() => {
    api<{ following: string[] }>("/api/social/following").then((res) => {
      setList(Array.isArray(res.following) ? res.following : []);
    }).catch(() => setList([]));
  }, []);
  ...
  return (
    <main className="mx-auto max-w-3xl px-3 py-5 sm:px-5">
      <h1 className="text-3xl font-bold">{t("social.following")}</h1>
      <div className="mt-3 rounded-xl border border-slate-700 p-3">
        <div className="space-y-1 text-sm">
          {pageItems.length ? pageItems.map((id) => (
            <div key={`following_${id}`} className="truncate rounded bg-slate-800/60 px-2 py-1">{id}</div>
